    ...
    pub fn new(value: T) -> NonNull<Self> {
        GC_STATE.with(|st| {
            ...
            if st.bytes_allocated > st.threshold {
                ...
                if st.bytes_allocated as u64 > st.threshold as u64 * USED_SPACE_RATIO / 100 {
                    ...
                    st.threshold = (st.bytes_allocated as u64 * 100 / USED_SPACE_RATIO) as usize
                }
            }
